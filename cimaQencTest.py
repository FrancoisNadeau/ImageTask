# -*- coding: utf-8 -*-
"""
Created on Tue Oct  8 21:52:05 2019
...
#        self.outputlist.append(self.TrialDict)
    ...
    def runEnc(self,whichTrial):
        ...
        for elem in self.Distractors :
            self.EStims.insert(rb(len(EStims)), elem)
        for stim in range(int(len(self.thisEncTrial)*1.5)-1):
            if randSign() == 1:
                for imStim in range(len(self.thisEncTrial)):
                    encstim = visual.ImageStim(self.win,
                                                self.thisEncTrial[imStim],
                                                color=(1,1,1), 
                                                pos = self.setstimpos(), 
                                                size = (500, 500),
                                                name=self.thisEncTrial[imStim])
            else:
                encstim = visual.ImageStim(self.win,
                                            self.categs.graySquare,
                                            color=(1,1,1), 
                                            pos = self.setstimpos(), 
                                            size = (500, 500),
                                            name=self.categs.graySquare)
            encstim.draw()
            self.win.flip()
...
#        self.stimDF.to_csv(os.getcwd()+'\\stimDF2.csv')
        ...
        #TODO: remove control stims from results
